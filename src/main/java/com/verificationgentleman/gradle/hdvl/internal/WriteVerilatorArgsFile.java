...
public abstract class WriteVerilatorArgsFile implements TransformAction<TransformParameters.None> {
    @InputArtifact
    ...
    public void transform(TransformOutputs outputs) {
        ...
        File verilatorArgsFile = outputs.file(input.getName() + ".verilator_args.f");
        DefaultHDVLCompileSpec compileSpec = getCompileSpec(input);
        writeVerilatorArgsFile(verilatorArgsFile, compileSpec);
    }
    ...
    private static void writeVerilatorArgsFile(File verilatorArgsFile, HDVLCompileSpec compileSpec) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(verilatorArgsFile, true))) {
